...
class Random(_random.Random):
    ...
    def uniform(self, a: float | int, b: float | int) -> float: ...
    def triangular(self, low: float | int = 0.0, high: float | int = 1.0, mode: float | int | None = None) -> float: ...
    if sys.version_info >= (3, 12):
        def binomialvariate(self, n: int = 1, p: float | int = 0.5) -> int: ...

    def betavariate(self, alpha: float | int, beta: float | int) -> float: ...
    if sys.version_info >= (3, 12):
        def expovariate(self, lambd: float | int = 1.0) -> float: ...
    else:
        def expovariate(self, lambd: float | int) -> float: ...

    def gammavariate(self, alpha: float | int, beta: float | int) -> float: ...
    if sys.version_info >= (3, 11):
        def gauss(self, mu: float | int = 0.0, sigma: float | int = 1.0) -> float: ...
        def normalvariate(self, mu: float | int = 0.0, sigma: float | int = 1.0) -> float: ...
    else:
        def gauss(self, mu: float | int, sigma: float | int) -> float: ...
        def normalvariate(self, mu: float | int, sigma: float | int) -> float: ...

    def lognormvariate(self, mu: float | int, sigma: float | int) -> float: ...
    def vonmisesvariate(self, mu: float | int, kappa: float | int) -> float: ...
    def paretovariate(self, alpha: float | int) -> float: ...
    def weibullvariate(self, alpha: float | int, beta: float | int) -> float: ...
