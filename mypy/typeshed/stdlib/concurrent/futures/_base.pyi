...
class Future(Generic[_T]):
    ...
    def result(self, timeout: float | int | None = None) -> _T: ...
    def set_running_or_notify_cancel(self) -> bool: ...
    def set_result(self, result: _T) -> None: ...
    def exception(self, timeout: float | int | None = None) -> BaseException | None: ...
...
class Executor:
    ...
    def map(
        self, fn: Callable[..., _T], *iterables: Iterable[Any], timeout: float | int | None = None, chunksize: int = 1
    ) -> Iterator[_T]: ...
    if sys.version_info >= (3, 9):
...
class _AsCompletedFuture(Protocol[_T_co]):
    ...
    def result(self, timeout: float | int | None = None) -> _T_co: ...

def as_completed(fs: Iterable[_AsCompletedFuture[_T]], timeout: float | int | None = None) -> Iterator[Future[_T]]: ...
...
if sys.version_info >= (3, 9):
    def wait(
        fs: Iterable[Future[_T]], timeout: float | int | None = None, return_when: str = "ALL_COMPLETED"
    ) -> DoneAndNotDoneFutures[_T]: ...

else:
    def wait(
        fs: Collection[Future[_T]], timeout: float | int | None = None, return_when: str = "ALL_COMPLETED"
    ) -> DoneAndNotDoneFutures[_T]: ...
