...
def store_parameter_type(
    ...
    if typ.arg_kinds[i] == ARG_STAR:
        if isinstance(param_type, ParamSpecType):
            pass
        elif isinstance(param_type, UnpackType):
            unpacked_type = get_proper_type(param_type.type)
            if isinstance(unpacked_type, TupleType):
                # Instead of using Tuple[Unpack[Tuple[...]]], just use Tuple[...]
                param_type = unpacked_type
            elif (
            ...
            param_type = named_type("builtins.tuple", [param_type])
    elif typ.arg_kinds[i] == ARG_STAR2:
        if not isinstance(param_type, ParamSpecType) and not typ.unpack_kwargs:
            param_type = named_type("builtins.dict", [named_type("builtins.str", []), param_type])
    defn.arguments[i].variable.type = param_type
