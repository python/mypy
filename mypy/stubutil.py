...
class TypedFunctionSig(NamedTuple):
    name: str
    args: ArgList
    ret_type: str
...
def infer_sig_from_docstring(docstr: str, name: str) -> Optional[TypedFunctionSig]:
    if not docstr:
        return None
    ...
    if m:
        ...
        return TypedFunctionSig(
            name=name,
            args=infer_arg_sig_from_docstring(m.group(1)),
            ret_type=m.group(2).rstrip()
        )
