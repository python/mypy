...
class SubtypeVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        ...
        if isinstance(right, Instance):
            if State.is_cached_subtype_check(left, right):
                return True
            ...
            if left.type.mro is not None:
                for base in left.type.mro:
                    ...
                    if base._promote and is_subtype(
                        ...
                        State.record_subtype_cache_entry(left, right)
                        return True
    ...
    with pop_on_exit(assuming, left, right):
        for member in right.type.protocol_members:
            ...
            if IS_CLASS_OR_STATIC in superflags and IS_CLASS_OR_STATIC not in subflags:
                return False
    State.record_subtype_cache_entry(left, right, proper_subtype)
    return True
...
class ProperSubtypeVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        right = self.right
        if isinstance(right, Instance):
            if State.is_cached_subtype_check(left, right, proper_subtype=True):
                return True
            for base in left.type.mro:
                if base._promote and is_proper_subtype(base._promote, right):
                    State.record_subtype_cache_entry(left, right, proper_subtype=True)
                    return True
