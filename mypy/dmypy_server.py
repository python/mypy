...
class Server:
    ...
    def fine_grained_increment(self, sources: List[mypy.build.BuildSource]) -> Dict[str, Any]:
        ...
        omitted = self.find_omitted(sources)
    ...
    def find_omitted(self, sources: List[mypy.build.BuildSource]
                     ) -> List[mypy.build.BuildSource]:
        modules = {source.module for source in sources}
        return [source for source in self.previous_sources if source.module not in modules]
