...
DUPLICATE_TYPE_SIGNATURES: Final = ErrorMessage(
    "Function has duplicate type signatures", codes.SYNTAX
)
...
NAMEDTUPLE_EXPECTED_LIST_TUPLE_DEFAULTS: Final = ErrorMessage(
    "List or tuple literal expected as the defaults argument to namedtuple()"
)
...
NAMEDTUPLE_TOO_MANY_DEFAULTS: Final = ErrorMessage(
    "Too many defaults given in call to namedtuple()"
)
...
# TypeArgs
TYPEVAR_INVALID_TYPE_ARG: Final = ErrorMessage(
    'Type variable "{}" not valid as type argument value for "{}"', codes.TYPE_VAR
)
TYPE_ARG_INVALID_SUBTYPE: Final = ErrorMessage(
    'Type argument "{}" of "{}" must be a subtype of "{}"', codes.TYPE_VAR
)
TYPE_ARG_INVALID_VALUE: Final = ErrorMessage(
...
EXCEPT_EXPR_NOTNAME_UNSIPPORTED: Final = ErrorMessage(
...
FORMAT_STR_BYTES_USE_REPR_OLD: Final = ErrorMessage(
    "On Python 3 '%s' % b'abc' produces \"b'abc'\", not 'abc'; use '%r' % b'abc' if this is desired behavior",
    codes.STR_BYTES_PY3,
)
...
OVERLOAD_IMPLEMENTATION_IN_STUB: Final = ErrorMessage("An implementation for an overloaded function is not allowed in a stub file")
OVERLOAD_IMPLEMENTATION_LAST: Final = ErrorMessage("The implementation for an overloaded function must come last")
OVERLOAD_IMPLEMENTATION_REQUIRED: Final = ErrorMessage("An overloaded function outside a stub file must have an implementation")
FINAL_DEC_ON_OVERLOAD_ONLY: Final = ErrorMessage("@final should be applied only to overload implementation")
FINAL_DEC_STUB_FIRST_OVERLOAD: Final = ErrorMessage("In a stub file @final must be applied only to the first overload")
...
FINAL_DEC_WITH_METHODS_ONLY: Final = ErrorMessage("@final cannot be used with non-method functions")
...
RUNTIME_CHECKABLE_WITH_NON_PROPERTY: Final = ErrorMessage('@runtime_checkable can only be used with protocol classes')
BASES_MUST_HAVE_SINGLE_GENERIC_OR_PROTOCOL: Final = ErrorMessage('Only single Generic[...] or Protocol[...] can be in bases')
DUPLICATE_TYPEVARS_IN_GENERIC_OR_PROTOCOL: Final = ErrorMessage("Duplicate type variables in Generic[...] or Protocol[...]")
GENERIC_PROTOCOL_NOT_ALL_TYPEVARS: Final = ErrorMessage("If Generic[...] or Protocol[...] is present it should list all type variables")
...
CANNOT_DETERMINE_MRO: Final = ErrorMessage('Cannot determine consistent method resolution order (MRO) for "{}"')
INNER_METACLASS_UNSUPPORTED: Final = ErrorMessage("Metaclasses defined as inner classes are not supported")
...
UNSUPPORTED_NAMED_DYNAMIC_BASE_CLASS: Final = ErrorMessage('Dynamic metaclass not supported for "{}"')
INVALID_METACLASS: Final = ErrorMessage('Invalid metaclass "{}"')
METACLASS_MUST_INHERIT_TYPE: Final = ErrorMessage('Metaclasses not inheriting from "type" are not supported')
...
MODULE_MISSING_ATTIRBUTE: Final = ErrorMessage('Module "{}" has no attribute "{}"{}', codes.ATTR_DEFINED)
NO_IMPLICIT_REEXPORT: Final = ErrorMessage('Module "{}" does not explicitly export attribute "{}"; implicit reexport disabled')
INCORRECT_RELATIVE_IMPORT: Final = ErrorMessage("Relative import climbs too many namespaces")
INVALID_TYPE_ALIAS_TARGET: Final = ErrorMessage('Type variable "{}" is invalid as target for type alias')
NAMEDTUPLE_ATTRIBUTE_UNSUPPORTED: Final = ErrorMessage("NamedTuple type as an attribute is not supported")
NAMEDTUPLE_INCORRECT_FIRST_ARG: Final = ErrorMessage('First argument to namedtuple() should be "{}", not "{}"', codes.NAME_MATCH)
TYPEDDICT_ATTRIBUTE_UNSUPPORTED: Final = ErrorMessage("TypedDict type as attribute is not supported")
FINAL_ATMOST_ONE_ARG: Final = ErrorMessage("Final[...] takes at most one type argument")
FINAL_INITIALIZER_REQUIRED: Final = ErrorMessage("Type in Final[...] can only be omitted if there is an initializer")
FINAL_CLASSVAR_DISALLOWED: Final = ErrorMessage("Variable should not be annotated with both ClassVar and Final")
...
FINAL_ONLY_ON_SELF_MEMBER: Final = ErrorMessage("Final can be only applied to a name or an attribute on self")
FINAL_ONLY_IN_CLASS_BODY_OR_INIT: Final = ErrorMessage("Can only declare a final attribute in class body or __init__")
PROTOCOL_MEMBERS_MUST_BE_TYPED: Final = ErrorMessage('All protocol members must have explicitly declared types')
MULTIPLE_TYPES_WITHOUT_EXPLICIT_TYPE: Final = ErrorMessage('Cannot assign multiple types to name "{}" without an explicit "Type[...]" annotation')
TYPE_DECLARATION_IN_ASSIGNMENT: Final = ErrorMessage('Type cannot be declared in assignment to non-self attribute')
UNEXPECTED_TYPE_DECLARATION: Final = ErrorMessage('Unexpected type declaration')
STAR_ASSIGNMENT_TARGET_LIST_OR_TUPLE: Final = ErrorMessage('Starred assignment target must be in a list or tuple')
...
PROTOCOL_ASSIGNMENT_TO_SELF: Final = ErrorMessage("Protocol members cannot be defined via assignment to self")
STARTYPE_ONLY_FOR_STAR_EXPRESSIONS: Final = ErrorMessage('Star type only allowed for starred expressions')
...
TYPEVAR_CALL_EXPECTED_STRING_LITERAL: Final = ErrorMessage("{}() expects a string literal as first argument")
TYPEVAR_NAME_ARG_MISMATCH: Final = ErrorMessage('String argument 1 "{}" to {}(...) does not match variable name "{}"')
...
TYPEVAR_COVARIANT_MUST_BE_TRUE: Final = ErrorMessage("TypeVar 'covariant' may only be 'True'")
TYPEVAR_CONTRAVARIANT_MUST_BE_TRUE: Final = ErrorMessage("TypeVar 'contravariant' may only be 'True'")
TYPEVAR_VALUE_WITH_BOUND_DISALLOWED: Final = ErrorMessage("TypeVar cannot have both values and an upper bound")
...
USE_NEW_TYPEVAR_SYNTAX: Final = ErrorMessage("Use TypeVar('T', t, ...) instead of TypeVar('T', values=(t, ...))")
TYPEVAR_COVARIANT_AND_CONTRAVARIANT: Final = ErrorMessage("TypeVar cannot be both covariant and contravariant")
TYPEVAR_SINGLE_CONSTRAINT: Final = ErrorMessage("TypeVar cannot have only a single constraint")
CANNOT_DECLARE_TYPE_OF_PARAMSPEC: Final = ErrorMessage("Cannot declare the type of a parameter specification")
TYPE_EXPECTED: Final = ErrorMessage('Type expected')
CLASSVAR_OUTSIDE_CLASS_BODY: Final = ErrorMessage('ClassVar can only be used for assignments in class body')
MULTIPLE_MODULE_ASSIGNMENT: Final = ErrorMessage('Cannot assign multiple modules to name "{}" without explicit "types.ModuleType" annotation')
DELETABLE_MUST_BE_WITH_LIST_OR_TUPLE: Final = ErrorMessage('"__deletable__" must be initialized with a list or tuple expression')
DELETABLE_EXPECTED_STRING_LITERAL: Final = ErrorMessage('Invalid "__deletable__" item; string literal expected')
...
WITH_INCOMPATIBLE_TARGET_COUNT: Final = ErrorMessage('Incompatible number of types for "with" targets')
WITH_MULTIPLE_TYPES_EXPECTED: Final = ErrorMessage('Multiple types expected for multiple "with" targets')
...
LOCAL_DEFINITION_BEFORE_NONLOCAL: Final = ErrorMessage('Name "{}" is already defined in local scope before nonlocal declaration')
NAME_ONLY_VALID_IN_TYPE_CONTEXT: Final = ErrorMessage('"{}" is a type variable and only valid in type context')
SUPER_OUTSIDE_CLASS: Final = ErrorMessage('"super" used outside class')
INVALID_STAR_EXPRESSION: Final = ErrorMessage('Can use starred expression only as assignment target')
...
ANY_CALL_UNSUPPORTED: Final = ErrorMessage('Any(...) is no longer supported. Use cast(Any, ...) instead')
...
NONCONSECUTIVE_OVERLOAD_FOUND: Final = ErrorMessage("Nonconsecutive overload {} found")
