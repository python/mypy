...
class SemanticAnalyzer(NodeVisitor[None],
        ...
        if isinstance(decorator, RefExpr):
            if decorator.fullname in RUNTIME_PROTOCOL_DECOS:
                if defn.info.is_protocol:
                    ...
                    self.fail(message_registry.RUNTIME_CHECKABLE_WITH_NON_PROPERTY,
                              defn)
    ...
    def analyze_metaclass(self, defn: ClassDef) -> None:
        if defn.metaclass:
            ...
            if not isinstance(sym.node, TypeInfo) or sym.node.tuple_type is not None:
                self.fail(message_registry.INVALID_METACLASS.format(metaclass_name), defn.metaclass)
                return
    ...
    def check_typevarlike_name(self, call: CallExpr, name: str, context: Context) -> bool:
        ...
        if (not isinstance(call.args[0], (StrExpr, BytesExpr, UnicodeExpr))
            ...
            self.fail(message_registry.TYPEVAR_CALL_EXPECTED_STRING_LITERAL.format(typevarlike_type),
                      context)
            return False
    ...
    def process_typevar_parameters(self, args: List[Expression],
        ...
        for param_value, param_name, param_kind in zip(args, names, kinds):
            if not param_kind == ARG_NAMED:
                self.fail(message_registry.TYPEVAR_UNEXPECTED_ARG, context)
                return None
            if param_name == 'covariant':
                if isinstance(param_value, NameExpr):
                    if param_value.name == 'True':
                        ...
                        self.fail(message_registry.TYPEVAR_COVARIANT_MUST_BE_TRUE, context)
                        return None
                else:
                    self.fail(message_registry.TYPEVAR_COVARIANT_MUST_BE_TRUE, context)
                    return None
                ...
                if isinstance(param_value, NameExpr):
                    if param_value.name == 'True':
                        ...
                        self.fail(message_registry.TYPEVAR_CONTRAVARIANT_MUST_BE_TRUE, context)
                        return None
                else:
                    self.fail(message_registry.TYPEVAR_CONTRAVARIANT_MUST_BE_TRUE, context)
                    return None
                ...
                self.fail(message_registry.TYPEVAR_UNEXPECTED_ARG_NAMED.format(param_name), context)
                return None
        ...
        if not self.is_func_scope():
            ...
            if self.function_stack[-1].is_coroutine:
                if self.options.python_version < (3, 6):
                    self.fail(message_registry.YIELD_IN_ASYNC_FUNC, expr, serious=True, blocker=True)
                else:
                    self.function_stack[-1].is_generator = True
    ...
    def check_no_global(self,
        ...
        if name in self.globals:
            ...
            if is_overloaded_func and prev_is_overloaded:
                self.fail(message_registry.NONCONSECUTIVE_OVERLOAD_FOUND.format(name), ctx)
            elif prev_is_overloaded:
                self.fail(message_registry.DEFINITION_MISSING_OVERLOAD.format(name), ctx)
            else:
