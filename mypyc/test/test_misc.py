...
class TestHeaderDeps(unittest.TestCase):
    ...
    def test_cached_group_deps_populated_from_disk_cfile(self) -> None:
        # Reproduces the scenario where generate_c returns empty ctext for a
        # group (the "fully-cached" path), but the .c file from the previous
        # build is on disk.  Before the fix, get_header_deps was called with
        # empty content, returning no includes, so Extension.depends stayed
        # empty and cross-group header changes didn't trigger a .o recompile.
        #
        # Layout:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Without the fix: get_header_deps is called with empty ctext,
            # so per_cfile_deps gets an empty include list and Extension.depends
            # ends up empty — cross-group header changes go undetected.
            deps_without_fix: set[str] = set()
            for cfile_full, dep_names in [(consumer_c, get_header_deps([(consumer_c, "")]))]:
                deps_without_fix.update(
                    resolve_cfile_deps(os.path.dirname(cfile_full), dep_names, tmp)
                )
            assert deps_without_fix == set()

            # With the fix: the on-disk .c is read when ctext is empty, so
            # get_header_deps sees the real includes and the resolver walks
            # the transitive chain to find the cross-group header.
            try:
