...
from mypyc.ir.ops import StealsDescription, Value
...
COERCER = Callable[[Value, RType, int, bool], Value]  # See ll_builder.py
ARG_CONVERTER = Callable[[List[Value], COERCER], List[Value]]
CFunctionDescription = NamedTuple(
    'CFunctionDescription',  [('name', str),
                              ...
                              ('priority', int),
                              ('match', Callable[[List[RType], List[Value]], bool]),
                              ('arg_converter', Optional[ARG_CONVERTER])])
...
def method_op(name: str,
              ...
              priority: int = 1,
              match: Callable[[List[RType], List[Value]], bool] = default_match,
              arg_converter: Optional[ARG_CONVERTER] = None) -> CFunctionDescription:
    ...
    Args:
        ...
        match: select what kind of args this method accept
    """
    ...
    desc = CFunctionDescription(name, arg_types, return_type, var_arg_type, truncated_type,
                                ...
                                extra_int_constants, priority, match, arg_converter)
    ops.append(desc)
    return desc
...
def function_op(name: str,
                ...
                priority: int = 1,
                match: Callable[[List[RType], List[Value]], bool] = default_match,
                arg_converter: Optional[ARG_CONVERTER] = None
...
def custom_op(arg_types: List[RType],
    ...
    return CFunctionDescription('<custom>', arg_types, return_type, var_arg_type, truncated_type,
                                c_function_name, error_kind, steals, is_borrowed, ordering,
                                extra_int_constants, 0, match, arg_converter)
