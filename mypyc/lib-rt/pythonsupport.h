...
// methods claim the token on entry and drop it at every exit, which rejects both
// a reentrant resume (the generator, directly or indirectly, resuming itself) and
// a concurrent one from another thread. This mirrors what CPython's own
// generators do with gi_frame_state == FRAME_EXECUTING.
//
// On free-threaded builds the token additionally buys:
//
...
//
// There the token must be a real atomic read-modify-write, not a load followed by
// a store: two threads calling send() concurrently would both pass a non-atomic
...
// it also orders everything the previous holder of the token stored.
#ifdef Py_GIL_DISABLED
