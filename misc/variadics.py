...
def expand_template(
    template: str, arg_template: str = "arg{i}: {Ts}", lower: int = 0, limit: int = LIMIT
) -> None:
    print()
...
def main():
    ...
    # map()
    expand_template("def map(func: Callable[[{Ts}], R], {argsTs}) -> R: ...", lower=1)
    # zip()
    expand_template("def zip({argsTs}) -> Tuple[{Ts}]: ...")

    # Naomi's examples
    expand_template("def my_zip({argsTs}) -> Iterator[Tuple[{Ts}]]: ...", "arg{i}: Iterable[{Ts}]")
    expand_template("def make_check({argsTs}) -> Callable[[{Ts}], bool]: ...")
    expand_template(
        "def my_map(f: Callable[[{Ts}], R], {argsTs}) -> Iterator[R]: ...",
        "arg{i}: Iterable[{Ts}]",
    )
