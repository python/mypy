...
.. option:: --local-partial-types
    ...
    cases are variables initialized with ``None`` but without explicit ``Optional`` annotation,
    therefore their types can not be fully inferred and specified. For example:
