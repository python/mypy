...
``no_site_packages`` (bool, default False)
    Disables searching for :pep:`561` compliant packages. This will also
    disable searching for a usable Python executable. This acts the same as
    :ref:`--no-site-packages <no-site-packages>` command line flag.
