...
class LexerSuite(Suite):
    ...
    def test_keywords(self):
        self.assert_lex('if else elif def return pass', 'Keyword(if) Keyword( else) Keyword( elif) Keyword( def) ' + 'Keyword( return) Keyword( pass) Break() Eof()')
        
        self.assert_lex('from import as class global any', 'Keyword(from) Keyword( import) Keyword( as) Keyword( class) ' + 'Keyword( global) Keyword( any) ...')
        
        self.assert_lex('interface', 'Keyword(interface) ...')
    
    def test_identifiers(self):
        self.assert_lex('i x FooBar FOO_BAR __x var', 'Name(i) Name( x) Name( FooBar) Name( FOO_BAR) Name( __x) ' + 'Name( var) Break() Eof()')
    
    def test_int_literals(self):
        self.assert_lex('0 1 0987654321 10002000300040005000600070008000', 'IntLit(0) IntLit( 1) IntLit( 0987654321) ' + 'IntLit( 10002000300040005000600070008000) Break() Eof()')
    
    def test_hex_int_literals(self):
        self.assert_lex('0x0 0xabcedf0189 0xAFe 0X2', 'IntLit(0x0) IntLit( 0xabcedf0189) IntLit( 0xAFe) ' + 'IntLit( 0X2) ...')
    
    def test_oct_int_literals(self):
        self.assert_lex('0o0 0o127 0O1', 'IntLit(0o0) IntLit( 0o127) IntLit( 0O1) ...')
    
    def test_float_literals(self):
        self.assert_lex('1.2 .1 1.', 'FloatLit(1.2) FloatLit( .1) FloatLit( 1.) ...')
        
        self.assert_lex('1e2 1.2e+3 1.3e-12', 'FloatLit(1e2) FloatLit( 1.2e+3) FloatLit( 1.3e-12) ...')
    ...
    def test_operators(self):
        self.assert_lex('- + < > == != <= >= .', 'Op(-) Op( +) Op( <) Op( >) Op( ==) Op( !=) Op( <=) ' + 'Op( >=) Op( .) ...')
        
        self.assert_lex('* / % // **', 'Op(*) Op( /) Op( %) Op( //) Op( **) ...')
        
        self.assert_lex('& | ^ ~ << >>', 'Op(&) Op( |) Op( ^) Op( ~) Op( <<) Op( >>) ...')
        
        self.assert_lex('in is and or not', 'Op(in) Op( is) Op( and) Op( or) Op( not) ...')
    
    def test_punctuators(self):
        self.assert_lex(': = ,', 'Colon(:) Punct( =) Punct( ,) ...')
        
        self.assert_lex('+= -= *= %= //=', 'Punct(+=) Punct( -=) Punct( *=) Punct( %=) Punct( //=) ...')
    ...
    def test_basic_indentation(self):
        ...
        self.assert_lex('y' + '\n' + '  x' + '\n' + 'z', 'Name(y) Break(\\n) Indent(  ) Name(x) Break(\\n) Dedent() ' + 'Name(z) Break() Eof()')
    
    def test_multiple_indent_levels(self):
        self.assert_lex('y' + '\n' + '  x' + '\n' + '  y' + '\n' + '    z', 'Name(y) Break(\\n) ' + 'Indent(  ) Name(x) Break(\\n) ' + 'Name(  y) Break(\\n) ' + 'Indent(    ) Name(z) Break() ' + 'Dedent() Dedent() Eof()')
        
        self.assert_lex('y' + '\n' + '  x' + '\n' + '    z' + '\n' + '  y', 'Name(y) Break(\\n) ' + 'Indent(  ) Name(x) Break(\\n) ' + 'Indent(    ) Name(z) Break(\\n) ' + 'Dedent() Name(  y) Break() ' + 'Dedent() Eof()')
    
    def test_tab_indent(self):
        self.assert_lex('y' + '\n' + '\t' + 'x' + '\n' + '        y' + '\n' + ' ' + '\t' + 'z', 'Name(y) Break(\\n) ' + 'Indent(\\t) Name(x) Break(\\n) ' + 'Name(        y) Break(\\n) ' + 'Name( \\tz) Break() ' + 'Dedent() Eof()')
    ...
    def test_parens(self):
        ...
        self.assert_lex('()' + '\n' + ' y', 'Punct(() Punct()) Break(\\n) Indent( ) Name(y) Break() ' + 'Dedent() Eof()')
    ...
    def test_str_literals(self):
        self.assert_lex("'' 'foo_bar'", "StrLit('') StrLit( 'foo_bar') Break() Eof()")
        self.assert_lex('"" "foo_bar"', 'StrLit("") StrLit( "foo_bar") Break() Eof()')
    ...
    def test_triple_quoted_string_literals(self):
        ...
        self.assert_lex("1 '''x''y'''1", "IntLit(1) StrLit( '''x''y''') IntLit(1) ...")
    ...
    def test_raw_string(self):
        self.assert_lex("r'' r'foo bar'", "StrLit(r'') StrLit( r'foo bar') ...")
        self.assert_lex('r"" r"foo bar"', 'StrLit(r"") StrLit( r"foo bar") ...')
    ...
    def test_byte_strings(self):
        self.assert_lex("b'\\'' b'foo bar'", "StrLit(b'\\'') StrLit( b'foo bar') ...")
        self.assert_lex('b"\\"" b"foo bar"', 'StrLit(b"\\"") StrLit( b"foo bar") ...')
    ...
    def test_line_numbers(self):
        ...
        self.assert_line('a\\n b', [1, 1, 2, 2, 2, 2, 2])    # dedent, break
        self.assert_line('a\\n b\\nc', [1, 1, 2, 2, 2, 3, 3])      # dedent, c
